...
export const insertEmosional = async (identitasAnak_id, skor1) => {
  ...
  const hasil1 = skor1 >= 100 ? "Tunalaras" : "Tidak teridentifikasi";
  ...
  try {
    ...
    const { data: categoryData, error: categoryError } = await supabase
      ...
      .insert([
        { kategori: "Emosional", hasil: hasil1, skor: skor1 },
      ])
      .select();
    ...
    const { data: resultData, error: resultError } = await supabase
      ...
      .insert([
        {
          identitasAnak_id,
          jenis_hambatan: "Emosional",
          hasil: `Emosional: ${hasil1}`,
          skor: skor1,
        },
      ])
      .select()
